...
def load_data(data_dir: Union[str, List[List[str]]],
              ...
              n_bins: int = 256, 
              min_repeats: int = 6,
              bin_data : bool =True) -> Tuple[jnp.ndarray, jnp.ndarray]:
    """
    ...
    if bin_data : 
        ...
        activity_norms = jnp.linalg.norm(response_orig, axis=-1)
        normalization_factors = activity_norms / jnp.sqrt(angles_orig.shape[0])
        response = response_orig / normalization_factors[:, None]
        return response[reliable_cells, :], angles_orig
