...
from edgar.scoring.scoring import _get_params, _eval_loss, _optimize, default_evaluate
...
def main(project_name: str):
    ...
    for idx, program in enumerate(spec.seed_programs):
        ...
        # Simple JAX translation rules. Note this is NOT the translation a real run
        # uses: there, t_translate_seeds asks an LLM for model_jax. Substituting here
        # keeps the script free of API calls
        model_jax_code = model_code.replace("import numpy as np", "import jax.numpy as jnp")
        ...
        try:
            params_init = _get_params(param_est_fn, program.default_params, X_discover[0])
            print("✓ Estimated initial parameters successfully")
            for k, v in params_init.items():
                print(f"  - {k}: shape {v.shape}, mean {np.nanmean(v):.4e}")
        except Exception as e:
        ...
        try:
            initial_loss = _eval_loss(model_fn, spec.loss_fn, evaluate_fn, params_init, X_discover[1])
            print(f"Initial loss on discovery test split: {initial_loss:.4e}")
        except Exception as e:
        ...
        try:
            ...
            params_opt = _optimize(model_fn, spec.loss_fn, evaluate_fn, params_init, X_discover[0], gd_config)
        ...
        # Evaluate final loss
        try:
            final_loss = _eval_loss(model_fn, spec.loss_fn, evaluate_fn, params_opt, X_discover[1])
            print(f"Final loss on discovery test split: {final_loss:.4e}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Verify and score seed programs for a given project.")
    parser.add_argument("project_name", type=str, help="Name of the project directory under the projects/ directory.")
