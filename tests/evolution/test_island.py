...
from tests.evolution.utils import (
    make_empty_population,
    make_seeds,
    make_fingerprint_population,
    make_fingerprint_program,
)
...
def test_spawn():
    ...
    expected_parents = [
        ...
        [1, 3],
        [1, 3],
    ]  # island1 #from run with np.random.default_rng(42)
    for i, island in enumerate(islands):
...
def test_prune_missing_loss():
    losses = (
        ...
        1,
        0.6,
    )  # missing a loss for final program, treated as None so pruned out
...
def test_prune_less_than_critical_num_losses():
    losses = (
        ...
        10,
        0.6,
    )  # missing two losses, both treated as None, so one kept as inf
    population = make_empty_population(num_programs=6)
    for i, loss in enumerate(losses):
        population[i].program_losses.discover.final = loss
...
def test_uniform_sample_distribution():
    ...
    assert (
        p_value > 0.01
    )  # fails 1 in 100 times if distribution is uniform, but same seed makes deterministic
...
def test_boltzmann_sample_highT_isuniform():
    ...
    assert (
        p_value > 0.01
    )  # 1 in 100 times this would fail if distribution is actually uniform, since we use the same seed this is deterministic
...
def test_boltzmann_sample_None_least_sampled():
    ...
    assert counts[2] < min(
        counts[0], counts[1], counts[3], counts[4]
    )  # program with None loss should be least sampled
...
def test_migration_low_loss_migrated():
    ...
    evolution = {
        "n_migrants": 1,
        "topology": [1, 2, 0],
    }  # program with idx 0 migrates to island 1, program with idx 5 migrates to island 2, program with idx 7 migrates to island 0, note migration is copying
