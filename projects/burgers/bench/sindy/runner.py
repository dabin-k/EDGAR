...
Key validated facts (see coarsening_diagnostic.py and journal note 2026-07-20):
  * On a WELL-RESOLVED grid (fine, Nx=256) SINDy recovers u_t = 0.02 u_xx - 1.0 u u_x
    essentially exactly, at zero noise, at every time-subsampling we tried.
  * On the COARSE benchmark grid (Nx=64, s_factor=4) it FAILS even at zero noise:
    the u_xx term drops out and spurious terms appear. This is a coarse-graining
    (closure) failure, not a noise failure -- coarse-grained dynamics do not obey the
    fine-grid PDE. Noise then makes it worse.

CLI:
    python runner.py --noise 0.0            # fit on shared coarse bundle
