...
def plot_model_fits(
    ...
    *,
    ...
    for row, program in enumerate(programs, start=1):
        ...
        preds = np.asarray(preds)[sample, block]  # (n_starts, rollout_steps, n_sensors)
        targets = np.asarray(targets)[sample, block]
        residual = preds - targets

        # rollout geometry, read back from the evaluator's output shape:
        # target time of prediction (start s, horizon h) is s + 1 + h
        n_starts, rollout_steps = preds.shape[0], preds.shape[1]
        starts = (truth.shape[1] - rollout_steps - n_starts) + np.arange(n_starts)
        t_first = starts + 1
