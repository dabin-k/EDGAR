...
def model(state, y_prev, params):
    ...
    observation. Still a purely LINEAR oscillator — damping does not depend on
    position, so there is no mechanism to produce the (1 - x^2) * u nonlinear
    kick that separates fast and slow phases of a limit cycle. This is the
    strongest "seed floor" that any VdP-like evolved program must clearly beat
    before we can claim the nonlinear damping was discovered.
    """
    x = state["x"]
    v = state["v"]
