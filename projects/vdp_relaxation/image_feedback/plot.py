"""Diagnostic plots for the VdP state-space project.
...
def plot_model_fits(
    ...
    repo_root = save_p
    for _ in range(10):
        if (repo_root / "projects" / "vdp_relaxation").is_dir():
            break
        if repo_root.parent == repo_root:
            raise RuntimeError(
                f"couldn't locate repo root walking up from {save_p}; "
                "expected projects/vdp_relaxation/ somewhere above."
            )
        repo_root = repo_root.parent
    if str(repo_root) not in sys.path:
        sys.path.insert(0, str(repo_root))
    from projects.vdp_relaxation.data_loader.load_data import apply_model   # noqa: E402
    ...
    for row, s in enumerate(show_idx):
        ...
        ax.plot(np.arange(T_show + 1), y[s, :T_show + 1], "k-", lw=0.6, alpha=0.5, label="true x")
        ax.set_ylabel(f"traj {s}")
        if row == 0:
            ax.legend(fontsize=7, loc="upper right")
            ax.set_title(f"VdP state-space one-step mean vs true (first {T_show} bins)")
        if row == n_show - 1:
    ...
    fig.suptitle("vdp_relaxation fits\n" + "  |  ".join(title_parts), fontsize=10)
