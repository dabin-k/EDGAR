"""Diagnostic plots for the state-space oscillator project.

Shows one-step predictions (mean of the emitted Gaussian) overlaid on the true
trajectory for a few sample trajectories. Free-running rollout is not shown
because it requires re-running the scan with model-generated observations —
possible but out of scope for v1 diagnostics.
"""
...
def plot_model_fits(
    ...
    """One-step predicted mean vs true observation for a few trajectories."""
    if not save_path:
    ...
    y = np.asarray(data["y"])                   # (n_samples, T)
    n_samples, T = y.shape
    ...
    for row, s in enumerate(show_idx):
        ...
        for j, model_fn in enumerate(model_fns):
            ...
            try:
                ...
                sample_params = {k: jnp.asarray(np.asarray(v)[s:s + 1]) for k, v in params[j].items()}
        ...
        if row == 0:
            ...
            ax.set_title(f"State-space one-step mean vs true (first {T_show} bins)")
        if row == n_show - 1:
    ...
    fig.suptitle("oscillator_ss fits\n" + "  |  ".join(title_parts), fontsize=10)
