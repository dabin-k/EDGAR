...
if __name__ == "__main__":
    ...
    'W_EE' : 0.2,  # weight of excitatory to excitatory connections
    'W_IE' : 0.22,  # weight of inhibitory to excitatory connections
    'W_EI' : 0.15,  # weight of excitatory to inhibitory connections
    'W_II' : 0.14,  # weight of inhibitory to inhibitory connections
    'E_max' : 10.0,
    'I_max' : 10.0,
    'XE' : 0.1,
    'XI' : 0.1
    }
    E_0, I_0 = 1.0, 1.0 #Initial values are 1
    E_max, I_max, W_EE, W_EI, W_IE, W_II = params['E_max'], params['I_max'], params['W_EE'], params['W_EI'], params['W_IE'], params['W_II']
    #Choose C_E, C_I to satisfy steady-state equations (no stim, above threshold)
    params['C_E']= E_0 / (E_max - E_0) - W_EE * E_0 + W_EI * I_0
    params['C_I'] = I_0 / (I_max - I_0) - W_IE * E_0 + W_II * I_0
    print(f"Chosen C_E: {params['C_E']}, C_I: {params['C_I']} to satisfy steady-state equations for E_0={E_0}, I_0={I_0}")
    if params['C_E'] < 0 or params['C_I'] < 0:
        print("Warning: Chosen C_E or C_I is negative")
    tmax = 1000
    stim_t = 50
    ...
    axes[0].plot(ts, Et, label='E(t)', color = 'C0')
    axes[1].plot(ts, It, label='I(t)', color = 'C1')
