...
def _print_detail(name, fit, true_disc, disc_idx, train_l, test_l):
    """True-vs-optimised parameter table + per-sample train/test loss.

    Columns are the discover (train) samples; rows are the 12 parameters. For
    each sample we print the true value, the optimised value, and the signed
    relative error (fit-true)/|true|.
    """
    fit_a = _stack_params(fit)            # (12, n)
    true_a = true_disc.T                   # (12, n)
    n = true_a.shape[1]
    print(f"\n=== {name} : true vs optimised parameters ===")

    # Per-sample train/test loss (the held-out cross-validation comparison).
    print(f"  {'sample':<8}{'disc_row':>9}{'train_loss':>12}{'test_loss':>12}")
    for j in range(n):
        print(f"  s{j:<7}{int(disc_idx[j]):>9}{train_l[j]:>12.5f}{test_l[j]:>12.5f}")
    print(f"  {'MEAN':<8}{'':>9}{train_l.mean():>12.5f}{test_l.mean():>12.5f}")
...
def main() -> None:
    ...
    for name, p_init, _detail in schemes:
        ...
        print(f"{name:<32} {train_l.mean():>9.5f} {test_l.mean():>9.5f} "
              f"{np.median(re):>11.3f} {np.percentile(re, 90):>11.3f}")

    # ── Detail: true vs optimised parameter tables for the flagged inits ────
    for name, _p, detail in schemes:
        if detail:
            train_l, test_l = losses[name]
            _print_detail(name, fits[name], true_disc, disc_idx, train_l, test_l)
