...
def _load_synthetic(
    ...
    E_response = raw['E'] # (n_samples, n_stim, n_folds, T)
    I_response = raw['I'] # (n_samples, n_stim, n_folds, T)
    response = np.stack([E_response, I_response], axis=-1) # (n_samples, n_stim, n_folds, T, 2)
    ...
    n_samples, n_stim, n_folds, T, _ = response.shape
    anchor_starts, K = _rollout_anchors(T)
    A = len(anchor_starts)

    # 1. Dynamic CV Splitting
    if cv_type == "k_fold":
        train_list = []
        test_list = []
        for s in range(n_samples):
            for f in range(n_folds):
                test_trace = response[s, :, f, :, :]  # (n_stim, T, 2)
                if n_folds == 1:
                    train_trace = test_trace
                else:
                    other_folds = [r for r in range(n_folds) if r != f]
                    train_trace = np.mean(response[s, :, other_folds, :, :], axis=1)  # (n_stim, T, 2)
                train_list.append(train_trace)
                test_list.append(test_trace)
                
        train_data_split = np.stack(train_list, axis=0)  # (n_samples * n_folds, n_stim, T, 2)
        test_data_split = np.stack(test_list, axis=0)    # (n_samples * n_folds, n_stim, T, 2)
        train_stim = stimuli
        test_stim = stimuli
    ...
    perm = np.random.default_rng(sample_split_seed).permutation(n_total_samples)
    ...
    print(
        f"[wilson_cowan/synthetic] {data_path}: n_samples={n_samples}, n_folds={n_folds}, cv_type={cv_type}; "
        f"objective={os.environ.get('EDGAR_WC_OBJECTIVE', DEFAULT_OBJECTIVE).upper()}, "
        f"K={K}, anchors={A}; "
